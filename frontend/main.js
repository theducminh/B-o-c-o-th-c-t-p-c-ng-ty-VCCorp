...
async function fetchWeekEvents() {
  ...
  document.querySelectorAll('[data-day-offset]').forEach(c => {
    // remove old event blocks except header
  ...
  try {
    ...
    events.forEach(ev => {
      ...
      const topPx = startHour * 50;
      const heightPx = Math.max(durationH*50, 30);
      const evDiv = document.createElement('div');

      evDiv.className = 'event-block bg-indigo-500 text-white cursor-pointer';
      evDiv.style.top = `${topPx}px`;
      evDiv.style.height = `${heightPx - 2}px`;
      evDiv.innerHTML = `
        <div class="font-semibold truncate">${ev.title}</div>
        <div class="text-[9px]">${start.getHours()}:${String(start.getMinutes()).padStart(2,'0')} - ${end.getHours()}:${String(end.getMinutes()).padStart(2,'0')}</div>
      `;
  ...
  try {
    ...
    if (Array.isArray(tasks) && tasks.length) {
      tasks.forEach(t => {
        ...
        d.className = 'border rounded p-2 flex justify-between bg-gray-50';
        d.innerHTML = `
          <div class="flex-1">
          ...
          ${t.assigned_event_id ? `<div class="text-xs text-green-600">Linked</div>` : ''}
        `;
        list.appendChild(d);
      });
    } else {
  ...
  try {
    ...
    if (data.suggestion) {
      ...
      document.getElementById('apply-suggestion').onclick = async () => {
        // backend đã tạo event và trả assigned_event_id nếu có
